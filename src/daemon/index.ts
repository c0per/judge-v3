...
(async function () {
    ...
    await remote.waitForTask(async (task) => {
        ...
        try {
            await remote.reportProgress({ taskId: task.taskId, type: ProgressReportType.Started, progress: null });
            result = await judge(task, async (progress) => {
                await remote.reportProgress({ taskId: task.taskId, type: ProgressReportType.Progress, progress: progress });
            }, async (progress) => {
